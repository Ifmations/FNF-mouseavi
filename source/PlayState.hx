...
class PlayState extends MusicBeatState
{
	...
	private var sanityBarBGBG:AttachedSprite;
	private var sanityBarBG:AttachedSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'room':
				var bg:BGSprite = new BGSprite('phase1/back', -600, -200, 0.9, 0.9);
				add(bg);

				var strip:BGSprite = new BGSprite('phase1/strip', -600, -200, 0.9, 0.9);
				add(strip);

				var mirror:BGSprite = new BGSprite('phase1/mirror', -600, -200, 0.9, 0.9);
				add(mirror);

				var bed:BGSprite = new BGSprite('phase1/bed', -600, -200, 0.9, 0.9);
				add(bed);

				var pillow:BGSprite = new BGSprite('phase1/pillow', -600, -200, 0.9, 0.9);
				add(pillow);
		...
		if(!modchartSprites.exists('blammedLightsBlack')) { //Creates blammed light black fade in case you didn't make your own
			...
			blammedLightsBlack.makeGraphic(Std.int(FlxG.width * 3000), Std.int(FlxG.height * 3000), FlxColor.BLACK);
		...
		sanityBarBG = new AttachedSprite('sanitybar');
		sanityBarBG.y = 0;
		sanityBarBG.x = 1000;
		sanityBarBG.scrollFactor.set();
		sanityBarBG.visible = false;
		add(sanityBarBG);

		sanityBar = new FlxBar(sanityBarBG.x, sanityBarBG.y, BOTTOM_TO_TOP, Std.int(sanityBarBG.width), Std.int(sanityBarBG.height), this, 'songMisses', 0, 20);
		sanityBar.scrollFactor.set();
		sanityBar.visible = false;
		sanityBar.createFilledBar(0xffffffff, 0xFF000000);
		add(sanityBar);
		sanityBarBG.sprTracker = sanityBar;
		if(sanityBar.percent > 20) {
			health = 0;
		}
