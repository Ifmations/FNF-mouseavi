...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bgYellow:FlxSprite = new FlxSprite(1000, 1056).makeGraphic(FlxG.width, 386, 0xFFF9CF51);
		bgSprite = new FlxSprite(0, 56);
		bgSprite.antialiasing = ClientPrefs.globalAntialiasing;

		blob = new FlxSprite(0, 0);
		blob.frames = Paths.getSparrowAtlas('blob');
		blob.animation.addByPrefix('idle', "blob idle", 24);
		blob.antialiasing = ClientPrefs.globalAntialiasing;
		blob.animation.play('idle');
		blob.updateHitbox();
		blob.screenCenter(X);
		add(blob);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			var weekThing:MenuItem = new MenuItem(0, bgSprite.y + 0, WeekData.weeksList[i]);
		...
		for (i in 0...CoolUtil.difficulties.length) {
			var sprDifficulty:FlxSprite = new FlxSprite(leftArrow.x + 396, leftArrow.y).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.difficulties[i].toLowerCase()));
			sprDifficulty.x += (-2000 - sprDifficulty.width) / 2;
			sprDifficulty.ID = i;
		...
		rightArrow = new FlxSprite(leftArrow.x + 306, leftArrow.y);
